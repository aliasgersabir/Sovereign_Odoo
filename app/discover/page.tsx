...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/search?city=${encodeURIComponent(query)}&format=json&addressdetails=1&limit=8`);
        const results: Suggestion[] = await res.json();
        // Deduplicate by city name + country to avoid showing the same city multiple times
        const seen = new Set<string>();
        const unique = results.filter((s) => {
          const key = `${(s.name || s.address.city || s.address.town || "").toLowerCase()}-${(s.address.country || "").toLowerCase()}`;
        ...
        if (res.ok) {
          ...
          if (currencies) {
            ...
            try {
              ...
              const existing = searches.find((s: { city: string }) => s.city === name);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#09090b", color: "#f4f4f5", fontFamily: "'Inter', sans-serif", display: "flex", flexDirection: "column", overflowX: "hidden" }}>
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes revealUp {
          0% { opacity: 0; transform: translateY(30px) scale(0.95); }
          100% { opacity: 1; transform: translateY(0) scale(1); }
        }
        .animate-reveal { opacity: 0; animation: revealUp 0.7s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
      ...
      <section style={{ position: "relative", width: "100%", height: "80vh", minHeight: "620px", display: "flex", alignItems: "center", justifyContent: "center", paddingTop: "80px" }}>
        ...
        <div style={{ position: "relative", zIndex: 10, width: "100%", maxWidth: "1000px", padding: "0 24px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <h1 className="animate-reveal" style={{ fontSize: "clamp(2.5rem, 5vw, 4.2rem)", fontWeight: 800, color: "#fff", marginBottom: "14px", textAlign: "center", lineHeight: 1.1, textShadow: "0 0 40px rgba(16,185,129,0.4)" }}>Discover your next adventure</h1>
          <p className="animate-reveal delay-100" style={{ fontSize: "clamp(1rem, 2vw, 1.25rem)", color: "rgba(255,255,255,0.8)", marginBottom: "48px", textAlign: "center", maxWidth: "550px", lineHeight: 1.6 }}>Search any city on Earth. Plan your perfect trip.</p>

          {/* SEARCH BAR */}
          <div ref={searchRef} className="animate-reveal delay-200" style={{ width: "100%", maxWidth: "860px", backgroundColor: "rgba(24,24,27,0.85)", backdropFilter: "blur(24px)", border: (isFocused || showCal) ? "1px solid rgba(16,185,129,0.5)" : "1px solid rgba(255,255,255,0.1)", borderRadius: "60px", display: "flex", alignItems: "stretch", boxShadow: "0 20px 60px rgba(0,0,0,0.5), inset 0 0 20px rgba(16,185,129,0.05)", transition: "all 0.3s", position: "relative" }}>
      ...
      <main style={{ width: "100%", maxWidth: "1200px", margin: "0 auto", padding: "0 24px 80px" }}>
        {selectedCity && (
          <div className="animate-reveal">
            {/* City Hero Banner */}
            <div style={{ position: "relative", width: "100%", height: "380px", borderRadius: "24px", overflow: "hidden", marginBottom: "40px", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 30px 60px -12px rgba(0,0,0,0.6)" }}>
              {cityImg ? (
            ...
            {/* Activities */}
            <h3 className="animate-reveal delay-100" style={{ fontSize: "26px", fontWeight: 700, color: "#fff", marginBottom: "24px" }}>Things to do in {cityName}</h3>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "24px" }}>
              {[
                ...
                <div key={i} className="animate-reveal" style={{ animationDelay: `${0.2 + (i * 0.1)}s` }}>
                  <ActivityCard {...item} currSym={currSym} exRate={exRate} />
                </div>
              ))}
            </div>
          </div>
