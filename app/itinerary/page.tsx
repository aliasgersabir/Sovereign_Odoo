...
import { Plus, Trash2, Share2, Copy, Check, MapPin, Clock, GripVertical, Loader2 } from "lucide-react";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { saveItinerary as saveItineraryAction } from "@/actions/itinerary";

interface OverpassElement {
  tags?: { tourism?: string; name?: string; website?: string };
}
...
export default function ItineraryPage() {
  ...
  useEffect(() => {
    ...
    if (currentDest) {
      ...
      .then(data => {
        ...
        const museums = elements.filter((e: OverpassElement) => e.tags?.tourism === "museum" && e.tags?.name).slice(0, 3);
        const viewpoints = elements.filter((e: OverpassElement) => e.tags?.tourism === "viewpoint" && e.tags?.name).slice(0, 3);
          ...
          if (museums.length > 0) {
            newDays.push({
              id: 1, day: 1, title: `Culture in ${currentDest.city}`, activities: [
                ...
                ...museums.map((m: OverpassElement, i: number) => ({ id: 10+i, time: `${11 + i*3}:00`, name: `Visit ${m.tags!.name}`, notes: m.tags!.website || "Check ticket availability" }))
              ]
            });
          }
          if (viewpoints.length > 0) {
            newDays.push({
              id: 2, day: 2, title: "Sightseeing & Views", activities: [
                ...viewpoints.map((v: OverpassElement, i: number) => ({ id: 20+i, time: `${10 + i*4}:00`, name: `See ${v.tags!.name}`, notes: "Great photo spot!" }))
              ]
            });
          }
  ...
  useEffect(() => {
    // Only saving locally to state to prevent DB spam, DB sync happens on click
  }, [days]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const encoded = btoa(JSON.stringify({ dest, days }));
      const shareUrl = `${window.location.origin}/itinerary/${encoded.slice(0, 12)}?data=${encoded}`;
      navigator.clipboard.writeText(shareUrl);
      
      localStorage.setItem(`shared_${encoded.slice(0, 12)}`, JSON.stringify({ dest, days }));
      alert("Itinerary saved successfully to your profile and database! Share link copied to clipboard.");
    } catch (e) {
      console.error(e);
  ...
  const handleShare = async () => {
    await handleSave(); // Ensure it is saved and URL is generated before showing
    setShowShare(true);
  };
