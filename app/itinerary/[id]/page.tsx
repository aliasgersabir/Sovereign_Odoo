...
interface SharedData { dest: { city: string; country: string } | null; days: Day[] }
...
export default function SharedItinerary() {
  ...
  useEffect(() => {
    // Strategy 1: Try to decode full data from URL (new approach)
    try {
    ...
    } catch { /* Not base64 encoded — fall through to localStorage */ }

    // Strategy 2: Legacy short-key lookup from localStorage
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#09090b", color: "#f4f4f5", fontFamily: "'Inter', sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: "#71717a", fontSize: "16px", marginBottom: "24px" }}>This shared itinerary link may have expired or is invalid.</p>
          <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "14px 28px", background: "#10b981", color: "#000", borderRadius: "14px", fontWeight: 700, fontSize: "15px", textDecoration: "none", transition: "transform 0.2s" }}>
            ← Back to Home
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#09090b", color: "#f4f4f5", fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, height: "70px", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 10%", background: "rgba(9,9,11,0.9)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.05)", zIndex: 1000 }}>
        <Link href="/" style={{ fontSize: "1.3rem", fontWeight: 800, color: "#10b981", letterSpacing: "2px", textDecoration: "none" }}>TRAVELO</Link>
