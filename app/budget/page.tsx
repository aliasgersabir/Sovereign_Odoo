...
import { getExpenses, addExpense as addExpenseAction } from "@/actions/budget";
...
export default function BudgetPage() {
    ...
    async function fetchExpenses() {
      ...
      if (res.success && res.expenses) {
        setExpenses(res.expenses.map((e: any) => ({
          id: parseInt(e.id.replace(/\D/g,'').substring(0,8)) || Date.now(), // Fallback mapping for string UUID to number
          category: e.category,
          label: e.category, // Using category as label for DB items
          amount: e.amount
        })));
      }
    }
  ...
  const addExpense = async () => {
    ...
    setExpenses([
      ...expenses,
      { id: tempId, category: newCategory, label: newLabel.trim(), amount: parsedAmount },
    ]);
    ...
    // Save to database
    await addExpenseAction({ amount: parsedAmount, category: newCategory });
  };
